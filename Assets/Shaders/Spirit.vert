...
void main() // Vertex
{
	...
	float offsetFactorX = length(vec2(Position.y, Position.z)) * 0.1;
	float offsetFactorY = length(vec2(Position.x, Position.z)) * 0.1;
	float offsetFactorZ = length(vec2(Position.x, Position.y)) * 0.1;
	
	float wobbleFactorX = sin(SimplexNoise(vec2(FragPos.y, FragPos.z) * 1) + Time) * offsetFactorX;
	float wobbleFactorY = sin(SimplexNoise(vec2(FragPos.x, FragPos.z) * 1) + Time) * offsetFactorY;
	float wobbleFactorZ = sin(SimplexNoise(vec2(FragPos.x, FragPos.y) * 1) + Time) * offsetFactorZ;
	
	vec3 wobble = vec3(wobbleFactorX, wobbleFactorY, wobbleFactorZ);
	
	FragPos = (ModelMatrix * Position + vec4(wobble, 0)).xyz;
	...
	gl_Position = ProjectionViewModel * inverse(ModelMatrix) * (ModelMatrix * Position + vec4(wobble, 0));
}
